...
from prepare import COLORS, MOVES, WIDTH_UNIT, MODES
...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def __init__(self, x, y, circuit_grid_model, game_mode):
        self.x = x
        self.y = y
    ...
    def move_to_adjacent_node(self, direction):
        if direction == MOVES["LEFT"] and self.selected_column > 0:
            self.selected_column -= 1
        elif (
            direction == MOVES["RIGHT"]
            and self.selected_column < self.circuit_grid_model.max_columns - 2
        ):
            self.selected_column += 1
        elif direction == MOVES["UP"] and self.selected_wire > 0:
...
class CircuitGridBackground(pygame.sprite.Sprite):
    ...
    def __init__(self, circuit_grid_model, game_mode):
        pygame.sprite.Sprite.__init__(self)

        self._background_width = (
            GRID_WIDTH // 2 if game_mode == MODES["TWO_PLAYER"] else GRID_WIDTH
        )
